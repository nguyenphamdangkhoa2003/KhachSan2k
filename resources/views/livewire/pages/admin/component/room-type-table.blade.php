<div>
    <x-mary-toast />
    <x-mary-header title="Type Room" subtitle="Type room in our hotel" separator>
        <x-slot:middle class="!justify-end">
            <x-mary-input icon="o-bolt" placeholder="Search..." wire:model.live="search" clearable />
        </x-slot:middle>
        <x-slot:actions>
            <x-mary-button class="btn-primary" wire:click="showAdd" label="Add" icon-right="o-plus" spinner="showAdd" />
        </x-slot:actions>
    </x-mary-header>
    ...
    <x-mary-table :headers="$headers" :rows="$type_rooms" :sort-by="$sortBy" with-pagination striped >
        ...
        @scope('cell_name', $type_rooms)
            <p>{{ $type_rooms->name }}</p>
        @endscope

        @scope('cell_price', $type_rooms)
            <p>{{ $type_rooms->price }}</p>
        @endscope

        @scope('cell_adult', $type_rooms)
            <p>{{ $type_rooms->adult }}</p>
        @endscope

        @scope('cell_children', $type_rooms)
            <p>{{ $type_rooms->children }}</p>
        @endscope
        {{-- Special `actions` slot --}}
        @scope('actions', $type_rooms)
            <div class="flex">
                <x-mary-button icon="o-trash" wire:click="showConfirm({{ $type_rooms->id }})"
                    spinner="showConfirm({{ $type_rooms->id }})" class="btn-sm" />
                <x-mary-button icon="o-pencil-square" wire:click="showEdit({{ $type_rooms->id }})"
                    spinner="showEdit({{ $type_rooms->id }})" class="btn-sm" />
            </div>
        @endscope
        <x-slot:empty>
    ...
    <x-mary-modal wire:model="isShowActionModal" class="backdrop-blur">
        <x-mary-form wire:submit="{{ $isEditing ? 'update' : 'save' }}">
            <x-mary-header title="{{ $isEditing ? 'Update Type Room' : 'Create Type Room' }}" separator
                progress-indicator="save" class="mb-0" />
            <x-mary-errors title="Oops!" description="Please, fix them." icon="o-face-frown" />
            <x-mary-input label="Name" wire:model="roomTypeForm.name" placeholder="Type room name" clearable />
            <x-mary-input type="number" label="Price" wire:model="roomTypeForm.price" prefix="VNĐ" min="0" />
            <div class="flex justify-between">
                <x-mary-input type="number" label="Adult" wire:model="roomTypeForm.adult" min="0" />
                <x-mary-input type="number" label="Children" wire:model="roomTypeForm.children" min="0" />
            </div>
