<div x-data="{
    ...
    <x-mary-header title="Register" separator progress-indicator="save" />
    <div class="grid lg:grid-cols-2 gap-5">
        <div>
            <x-mary-toast />
            <x-mary-form wire:submit.prevent="register">
                ...
                <x-mary-password wire:model="userForm.cpassword" label="Confirm password" hint="Confirm Password"
                    clearable />
                <x-slot:actions>
                    <x-mary-button label="Register" icon="" class="btn-primary" type="submit" spinner="save" />
                </x-slot:actions>

            </x-mary-form>
        </div>
        <div class="flex justify-center items-center">
            <x-svg.background class="w-full h-full" />
        </div>
    </div>
</div>
